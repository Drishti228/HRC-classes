...
public class Add extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int cust_number = request.getParameter("cust_number") != null ? Integer.parseInt(request.getParameter("cust_number")) : 0;
        Date clear_date = !request.getParameter("clear_date").equals("") ? Date.valueOf(request.getParameter("clear_date")) : null;
        ...
        int invoice_id = request.getParameter("invoice_id") != null ? Integer.parseInt(request.getParameter("invoice_id")) : 0;
		
		try {
			Crud addData = new Crud();
			Connection con = addData.getConnection();
			
			String query = "INSERT INTO winter_internship_updated (business_code, cust_number, clear_date, buisness_year, doc_id, posting_date, document_create_date, due_in_date, invoice_currency, document_type, posting_id, total_open_amount, baseline_create_date, cust_payment_terms, invoice_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
